...
data = json.load(open('dados/acougues/acougues-bh-enriquecido.json'))
...
# Stats
total = len(data)
boutique = sum(1 for r in data if 'boutique' in r['Porte estimado'].lower())
atacado = sum(1 for r in data if 'atacado' in r['Porte estimado'].lower())
referencia = sum(1 for r in data if 'referência' in r['Porte estimado'].lower())
consolidado = sum(1 for r in data if 'consolidado' in r['Porte estimado'].lower())
bairro = sum(1 for r in data if 'bairro' in r['Porte estimado'].lower())

html_out = f"""<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<title>Prospecção Açougues BH — L.M Agência</title>
<style>
...
<header>
  ...
  <div class="meta-info">
    PROSPECÇÃO · AÇOUGUES · BELO HORIZONTE<br>
    Fonte: Google Maps via Apify + enriquecimento IA
  </div>
</header>

<h1>30 açougues em BH<br>com <span class="accent">lógica de abordagem pronta.</span></h1>
...
<div class="stats">
  ...
  <div class="stat"><div class="num">{atacado}</div><div class="lbl">Atacado / distribuição</div></div>
  <div class="stat"><div class="num">{boutique}</div><div class="lbl">Boutiques de carnes</div></div>
  <div class="stat"><div class="num">{referencia + consolidado}</div><div class="lbl">Consolidados / referência</div></div>
  <div class="stat"><div class="num">{bairro}</div><div class="lbl">Açougues de bairro</div></div>
</div>
...
<div class="disclaimer">
  <strong>Sobre as estimativas:</strong> consumo e conta de luz são faixas empíricas baseadas no porte aparente do negócio (boutique, atacado, bairro) e benchmarks de mercado pra açougue no Brasil — refrigeração 24/7 é o componente dominante. Não são números reais da unidade. Servem como ponto de partida pra abordagem — o número exato vem com o diagnóstico.
</div>
...
with open('dados/acougues/dashboard-acougues-bh.html', 'w', encoding='utf-8') as f:
    f.write(html_out)

print('OK: dados/acougues/dashboard-acougues-bh.html')
