...
export default function Login() {
  ...
  async function entrar() {
    ...
    setCarregando(false);
    if (!r.ok) return setErro(d.erro || "erro ao entrar");
    router.push("/painel");
  }
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        <h1 style={{ margin: "0 0 4px", fontSize: 26 }}>⚡ Prospecta</h1>
        <p style={{ color: "#8b93a7", margin: "0 0 20px", fontSize: 14 }}>Sua IA prospecta no WhatsApp.</p>
        ...
        <button style={btn} onClick={entrar} disabled={carregando}>{carregando ? "entrando…" : "Entrar"}</button>
        {erro && <div style={{ color: "#ff6b6b", fontSize: 13, marginTop: 10 }}>{erro}</div>}
        <p style={{ fontSize: 13, color: "#8b93a7", marginTop: 18 }}>
          não tem conta? <Link href="/cadastro" style={{ color: "#6cc24a" }}>criar agora</Link>
        </p>
      </div>
    </div>
  );
